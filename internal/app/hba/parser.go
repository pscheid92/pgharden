...
func parseFile(fsys fs.FS, path string, depth int) ([]Entry, error) {
	...
	f, err := fsys.Open(domain.FSPath(path))
	if err != nil {
		return nil, err
	}
...
func parseIncludeDir(fsys fs.FS, dir, parentPath string, depth int) ([]Entry, error) {
	...
	dirEntries, err := fs.ReadDir(fsys, domain.FSPath(dir))
	if err != nil {
		return nil, err
	}
...
func LoadFromSQL(ctx context.Context, db domain.DBQuerier) ([]Entry, error) {
