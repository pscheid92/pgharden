...
func (c *check_2_1) Run(ctx context.Context, env *domain.Environment) (*domain.CheckResult, error) {
	result := domain.NewResult(domain.SeverityWarning)

	out, err := env.GetCmd().Run(ctx, "sh", "-c", "umask")
	if err != nil {
...
func (c *check_2_2) Run(ctx context.Context, env *domain.Environment) (*domain.CheckResult, error) {
	result := domain.NewResult(domain.SeverityWarning)

	dynPath, err := domain.ShowSetting(ctx, env.DB, "dynamic_library_path")
	if err != nil {
		return nil, err
	}
	...
	for _, dir := range dirs {
		...
		info, err := fs.Stat(fsys, domain.FSPath(dir))
		if err != nil {
			continue
		}
...
func (c *check_2_3) Run(ctx context.Context, env *domain.Environment) (*domain.CheckResult, error) {
	result := domain.NewResult(domain.SeverityWarning)
	...
	for _, base := range searchDirs {
		entries, err := fs.ReadDir(fsys, domain.FSPath(base))
		if err != nil {
			continue
		}
		...
		for _, entry := range entries {
			...
			info, err := fs.Lstat(fsys, domain.FSPath(histPath))
			if err != nil {
				continue
			}

			if info.Mode()&fs.ModeSymlink != 0 {
				target, err := fs.ReadLink(fsys, domain.FSPath(histPath))
...
func (c *check_2_4) Run(ctx context.Context, env *domain.Environment) (*domain.CheckResult, error) {
...
func checkServiceConf(fsys fs.FS, path string, problems *[]string) {
	data, err := fs.ReadFile(fsys, domain.FSPath(path))
	if err != nil {
		return
	}
...
func (c *check_2_5) Run(ctx context.Context, env *domain.Environment) (*domain.CheckResult, error) {
	result := domain.NewResult(domain.SeverityCritical)

	hbaFile, err := domain.ShowSetting(ctx, env.DB, "hba_file")
	if err != nil {
		return nil, err
	}

	info, err := fs.Stat(env.GetFS(), domain.FSPath(hbaFile))
	if err != nil {
...
func (c *check_2_6) Run(ctx context.Context, env *domain.Environment) (*domain.CheckResult, error) {
	result := domain.NewResult(domain.SeverityWarning)

	sockDirs, err := domain.ShowSetting(ctx, env.DB, "unix_socket_directories")
	if err != nil {
		return nil, err
	}
...
func (c *check_2_7) Run(ctx context.Context, env *domain.Environment) (*domain.CheckResult, error) {
	result := domain.NewResult(domain.SeverityCritical)

	info, err := fs.Stat(env.GetFS(), domain.FSPath(env.DataDir))
	if err != nil {
...
func (c *check_2_8) Run(ctx context.Context, env *domain.Environment) (*domain.CheckResult, error) {
	return domain.ManualResult("Manually verify that all files in " + env.DataDir + " are owned by the postgres OS user"), nil
}
