...
func newMockEnv(t *testing.T) (pgxmock.PgxConnIface, *domain.Environment) {
	...
	return mock, &domain.Environment{DB: mock, PGVersion: 16, HBALoaded: true}
}
...
// --- 5.1: passwords in process listing ---
...
func TestCheck_5_1_NoPasswords(t *testing.T) {
	_, env := newMockEnv(t)
	env.Commands = map[string]bool{"ps": true}
	env.Cmd = &mockCmd{
		outputs: map[string]string{
			"ps -ef": "UID  PID  PPID CMD\nroot 1 0 /sbin/init\npostgres 100 1 /usr/lib/postgresql/16/bin/postgres\n",
		},
	}
...
func TestCheck_5_1_PasswordFound(t *testing.T) {
	_, env := newMockEnv(t)
	env.Commands = map[string]bool{"ps": true}
	env.Cmd = &mockCmd{
		outputs: map[string]string{
			"ps -ef": "UID  PID  PPID CMD\nroot 1 0 /sbin/init\nuser 200 1 psql PGPASSWORD=secret -h db\n",
		},
	}
	...
	if result.Status != domain.StatusFail {
		t.Errorf("expected FAIL for PGPASSWORD in ps output, got %s", result.Status)
	}
}
