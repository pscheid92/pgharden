...
func TestCheck_1_2_ServiceEnabled(t *testing.T) {
	_, env := newMockEnv(t)
	env.Commands = map[string]bool{"systemctl": true}
	env.Cmd = &mockCmd{
		outputs: map[string]string{
	...
	if result.Status != domain.StatusPass {
		t.Errorf("expected PASS for enabled service, got %s", result.Status)
	}
}

func TestCheck_1_2_ServiceDisabled(t *testing.T) {
	_, env := newMockEnv(t)
	env.Commands = map[string]bool{"systemctl": true}
	env.Cmd = &mockCmd{
		outputs: map[string]string{
	...
	if result.Status != domain.StatusFail {
		t.Errorf("expected FAIL for disabled service, got %s", result.Status)
	}
}

func TestCheck_1_2_FallbackGlob(t *testing.T) {
	_, env := newMockEnv(t)
	env.Commands = map[string]bool{"systemctl": true}
	env.Cmd = &mockCmd{
		outputs: map[string]string{
			"systemctl is-enabled postgresql":              "disabled",
			"bash -c systemctl is-enabled postgresql*": "enabled",
		},
		errors: map[string]error{
			"systemctl is-enabled postgresql": fmt.Errorf("exit status 1"),
		},
	}
	...
	if result.Status != domain.StatusPass {
		t.Errorf("expected PASS via glob fallback, got %s", result.Status)
	}
}
