...
var (
	sqlOnly         = domain.CheckRequirements{SQLOnly: true}
	sqlOnlyCloudNA  = domain.CheckRequirements{SQLOnly: true, SkipPlatforms: domain.ManagedCloud}
	sqlOnlyLocalNA  = domain.CheckRequirements{SQLOnly: true, SkipPlatforms: domain.NonBareMetal}
)
...
func (c *check_3_1_3) Run(ctx context.Context, env *domain.Environment) (*domain.CheckResult, error) {
	val, err := domain.ShowSetting(ctx, env.DB, "logging_collector")
	if err != nil {
		return nil, err
	}
	...
	if env.Platform != domain.PlatformBareMetal {
...
func (c *check_3_2) Run(ctx context.Context, env *domain.Environment) (*domain.CheckResult, error) {
	val, err := domain.ShowSetting(ctx, env.DB, "shared_preload_libraries")
	...
	auditLog, err := domain.ShowSetting(ctx, env.DB, "pgaudit.log")
	if err != nil {
...
func (c *check_3_2) checkViaExtension(ctx context.Context, env *domain.Environment) (*domain.CheckResult, error) {
	var count int
...
func (c *check_3_1_22) Run(ctx context.Context, env *domain.Environment) (*domain.CheckResult, error) {
	val, err := domain.ShowSetting(ctx, env.DB, "log_line_prefix")
	if err != nil {
		return nil, err
	}
