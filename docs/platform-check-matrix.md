...
The **local** column indicates whether the check requires the `--local` flag (filesystem access or local commands). **Y** = requires `--local`, **~** = tries SQL first via `pg_read_file` (needs `pg_read_server_files` role), falls back to `--local`, blank = SQL-only or manual (no `--local` needed).

## Section 1: Installation and Patches

| Check | What it does | bare-metal | local | container | zalando | rds | aurora |
|-------|-------------|:---:|:---:|:---:|:---:|:---:|:---:|
| 1.1 | Package repos (manual) | M | | X | X | X | X |
| 1.1.1 | PGDG repos (rpm/dpkg) | M | | X | X | X | X |
| 1.2 | systemd service enabled | Y | Y | X | X | X | X |
...
| 1.4.4 | WAL/temp separate storage | Y | Y | X | X | X | X |
| 1.4.5 | Storage type audit (manual) | M | | M | M | X | X |
| 1.5 | Latest PG version (manual) | M | | M | M | M | M |
| 1.6 | PGPASSWORD in shell profiles | Y | Y | X | X | X | X |
| 1.7 | PGPASSWORD in process env | Y | Y | X | X | X | X |
...
| Check | What it does | bare-metal | local | container | zalando | rds | aurora |
|-------|-------------|:---:|:---:|:---:|:---:|:---:|:---:|
| 2.1 | umask is 0077 | Y | Y | X | X | X | X |
| 2.2 | pg_wheel group / ext dir perms | Y | Y | X | X | X | X |
| 2.3 | .psql_history disabled | Y | Y | X | X | X | X |
| 2.4 | .pg_service.conf no passwords | Y | Y | X | X | X | X |
| 2.5 | pg_hba.conf permissions | Y | Y | X | X | X | X |
| 2.6 | Unix socket dir permissions | Y | Y | X | X | X | X |
| 2.7 | PGDATA permissions 0700 | Y | Y | X | X | X | X |
| 2.8 | PGDATA owned by postgres | Y | Y | X | X | X | X |
...
| Check | What it does | bare-metal | local | container | zalando | rds | aurora |
|-------|-------------|:---:|:---:|:---:|:---:|:---:|:---:|
| 4.1 | postgres user no login shell | Y | Y | X | X | X | X |
...
| Check | What it does | bare-metal | local | container | zalando | rds | aurora |
|-------|-------------|:---:|:---:|:---:|:---:|:---:|:---:|
| 5.1 | No passwords in ps output | Y | Y | X | X | X | X |
...
| 6.7 | FIPS mode enabled | Y | Y | X | X | X | X |
...
| Check | What it does | bare-metal | local | container | zalando | rds | aurora |
|-------|-------------|:---:|:---:|:---:|:---:|:---:|:---:|
| 8.2 | pgBackRest configured | Y | Y | Y | X | X | X |
